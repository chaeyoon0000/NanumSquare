  ...
  <body>
    ...
    <section class="ftco-section">
    	...
    	<c:if test="${sessionScope.userId != null}">
    		<input type="submit" value="Register" onClick="location.href='<%=request.getContextPath()%>/views/game/form'" 
    				...
    				<div class="car-wrap ftco-animate">
    					<div class="img d-flex align-items-end" style="background-image: url(<%=request.getContextPath()%>/images/dd.jpg);">
    						<div class="price-wrap d-flex">
								<c:if test="${game.state eq '0'}"><span class="rate">Proceeding</span></c:if>
								<c:if test="${game.state eq '1'}"><span class="rate">Completed</span></c:if>
    							<p class="from-day">
    								<span>End Date/ </span>
    					...
    					<div class="text p-4 text-center">
    						<h2 class="mb-0"><a href="car-single.html">${game.gameNo}</a></h2>
    						<span>Status/ 
    							<br>
    							<c:choose>
									<c:when test="${game.maxPoint eq '0'}">User: None</c:when>
									<c:otherwise>User: ${game.id}</c:otherwise>
								</c:choose>
    							<br>Point: ${game.maxPoint}
    						</span>
    						<div class="col text-center">
    							<a href="<c:url value='/views/game/detail'><c:param name='gameNo' value='${game.gameNo}'/></c:url>" class="btn btn-black btn-outline-black ml-1">Go To Betting</a>
    						</div>
    					</div>
    				</div>
    			</div>
